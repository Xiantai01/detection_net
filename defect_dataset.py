...
class DEFDataSet(Dataset):
    ...
    def __init__(self, data_root,  train_list, transforms=None):
        self.img_root = os.path.join(data_root, "JPEGImages")
        self.annotations_root = os.path.join(data_root, "Annotations")
        self.lables = ['crazing_', 'inclusion_', 'patches_', 'pitted_surface_', 'rolled-in_scale_', 'scratches_']
        # read train.txt or val.txt file
        self.train_list = train_list
        self.xml_list = []
        xml_list = []

        for lables in self.lables:
            for i in self.train_list:
                xml_list.append(os.path.join(self.annotations_root, lables + str(i) + ".xml"))
        # check file
    ...
    def __getitem__(self, idx):
        ...
        xml = etree.fromstring(xml_str)
        data = self.parse_xml_to_dict(xml)["annotation"]
        name_error = data['filename']
        name_error = name_error.split('.')
        if len(name_error) == 1:
