...
def backbone_fpn(pretrain_path="",
    ...
    if Simsiam:
        checkpoint = torch.load('./backbone/ckpt_epoch_50_2_Resnet.pth')
        model_pre = SimSiam()
        model_pre.load_state_dict(checkpoint['state_dict'])
        resnet_backbone.load_state_dict(model_pre.backbone.state_dict(), strict=False)
